...
class GymAppointmentSchedule implements AppointmentSchedule {
  ...
  final _appointmentSubject = BehaviorSubject<List<AppointmentInfo>>();
  Stream<List<AppointmentInfo>> get onAppointments => _appointmentSubject;
  ...
  GymAppointmentSchedule({required this.appBloc, required this.gym}) {
    final appointmentsStream = FirebaseFirestore.instance
        ...
        .snapshots()
        .map((event) {
      return event.docs.map((e) => AppointmentInfo(e)).toList();
    });
    _appointmentSubject.addStream(appointmentsStream);
  }
